...
cv::Point3d GetTriangulatedPoint(cv::Mat P1, cv::Mat P2, cv::Point2f pt1, cv::Point2f pt2){
    cv::Mat A = cv::Mat::zeros(4, 4, CV_64F);
    ...
    cv::Point3d res(X1.at<double>(0, 0), X1.at<double>(1, 0), X1.at<double>(2, 0));
    return res;
}
...
static void DrawEpipolar(cv::Mat& img_1, cv::Mat& img_2, std::vector<cv::Point2f> prev_subset, std::vector<cv::Point2f> next_subset, cv::Matx33d F)
{
    cv::Mat img_full(img_1.rows, img_1.cols*2, CV_8UC3);

    cv::Rect left_full_img(0, 0, img_1.cols, img_1.rows);
    img_1.copyTo(img_full(left_full_img));

    cv::Rect right_full_img(img_1.cols, 0, img_1.cols, img_1.rows);
    img_2.copyTo(img_full(right_full_img));
    ...
    for(int i=0; i<prev_subset.size(); i++)
    {
        ...
        cv::Mat left = img_full(left_full_img);
        cv::Mat right = img_full(right_full_img);

        cv::Point s1_left(0,-c1/b1);
        cv::Point s2_left(img_1.cols, -(c1+a1*img_1.cols)/b1);
        cv::line(left, s1_left, s2_left, cv::Scalar(0,255,255));
        cv::circle(left, prev_subset[i], 5, cv::Scalar(0,0,255));

        cv::Point s1_right(0,-c2/b2);
        cv::Point s2_right(img_2.cols, -(c2+a2*img_2.cols)/b2);
        cv::line(right, s1_right, s2_right, cv::Scalar(0,255,255));
        cv::circle(right, next_subset[i], 5, cv::Scalar(0,0,255));
    }
    cv::imshow("Epipolar", img_full);
    cv::waitKey(0);
}

void GeneratePointTest(){
    ...
    std::cout << "pre defined Rotation Translation matrix" << std::endl;
    ...
    // whether to use E build with tx and Rotation 
    // or to use E computed by F from Findfundamental
    
    vector<int> img_size;
    img_size.push_back(1);
    img_size.push_back(1);
    Matx33d F = Findfundamental(points1,points2,img_size);
    E = K.t() * Mat(F) * K;
...
int main( int argc, char** argv )
{
    ...
    cout << "F" << endl;
    cout << F << endl;
    DrawEpipolar(img_1, img_2, prev_subset, next_subset, F);

    cv::Matx33d F_cv = (Matx33d)cv::findFundamentalMat(prev_subset, next_subset, cv::FM_8POINT, 1.5f);
    cout << "F_cv" << endl;
    cout << F_cv << endl;
    DrawEpipolar(img_1, img_2, prev_subset, next_subset, F_cv);
    ...
    // Get all triangulated 3D points
    vector<cv::Point3d> TriangulatedPoints;
    for (int i = 0; i < prev_subset.size(); i++)
        TriangulatedPoints.push_back(GetTriangulatedPoint(camera_P, P[chosen_idx], prev_subset[i], next_subset[i]));

    // Stimulation test FindFundamental function and Triangulation function
    GeneratePointTest();
